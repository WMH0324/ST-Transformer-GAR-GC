...
class GARMain:
    ...
    def train(self):
        ...
        bestResult = {'epoch': 0, 'activitiesAcc': 50}
        for epoch in range(self.config.train.startEpoch, self.config.train.totalEpochs):
            ...
            if self.epoch % 1 == 0:  # 控制测试的频率，epoch%2==0 是每2个epoch训练1次， epoch%3==0 是每3个epoch训练1次

                # 测试  每2个epoch训练1次
                valInfo = self.val()
                if self.epoch % 10 == 0:
                    filepath = self.config.resultPath + '/epoch%d_%.2f%%.pth' % (
                        epoch, valInfo['activitiesAcc'])  # 模型保存路径
                    self.saveModel(self.epoch, filepath)  # 保存模型参数
                show_epoch_info('Test', self.config.logPath, valInfo)  # 显示测试的相关参数信息
                test_accuracy.append(valInfo['activitiesAcc'])  # 将训练得到的准确率添加到test_accuracy中存储
                test_loss.append(valInfo['loss'])  # 将测试得到的损失值添加到test_loss中存储
                # for i in range( cfg.num_activities ):
                #     test_per_accuracy_total[i][epoch-1] = valInfo['activitiesPerclassAcc'][i]

                # 比较测试的准确率和目前最好的准确率，如果大于最好的准确率，则更新最好的准确率，同时保证此刻的模型参数
                if valInfo['activitiesAcc'] > bestResult['activitiesAcc']:
                    bestResult = valInfo  # 更新最好的准确率
                    filepath = self.config.resultPath + '/epoch%d_%.2f%%.pth' % (
                        epoch, valInfo['activitiesAcc'])  # 模型保存路径
                    self.saveModel(self.epoch, filepath)  # 保存模型参数
            if self.epoch % 1 == 0:
                print_log(self.config.logPath,  # 输出显示
                          'Best group activity accuracy: %.2f%% at epoch #%d.' % (
                              bestResult['activitiesAcc'], bestResult['epoch']))

            # 早停止
            early_stopping(valInfo['loss'], self.model)
        ...
        # #用于显示训练中每种类别的的准确率变化
        # showTrainPerAccuracy( train_per_accuracy_total )
        # #用于显示训练中每种类别的的准确率变化
        # showTestPerAccuracy( test_per_accuracy_total )
        # valResult = self.val()
        # if epoch % 10 == 0:
        #     filepath = self.config.checkPointPath + '/epoch%d_%.2f%%.pth' % (
        #         epoch, valResult['activitiesAcc'])  # 模型保存路径
        #     self.saveModel(epoch, filepath)
        #     print("save model finished!")
        #
        # if valResult['activitiesAcc'] > bestResult['activitiesAcc']:
        #     bestResult = valResult
        #     filepath = self.config.checkPointPath + '/epoch%d_%.2f%%.pth' % (
        #         epoch, bestResult['activitiesAcc'])  # 模型保存路径
        #     self.saveModel(epoch, filepath)
        #     print("save model finished!")
    ...
    def val(self):
        ...
        for batchData in self.valLoader:
            ...
            actionsScores, groupScores, activitiesScores, glb = self.model(batchData[0], batchData[1], batchData[4], batchData[6], batchData[7])
            groupIn = self.bulidGroupLabel(glb).cuda()
            # actionsIn为行人动作的标签  维度大小为（B, C） B为Batch,C为种类
            # activitiesIn为人群行为的标签  维度大小为(B,T) B为Batch，T为帧数
            # bboxesNum 为行人数量，大小为(B,T) B为Batch，T为帧数
    ...
    def buildCriterion(self):
        ...
        if config.train.criterion == 'ce_loss':
            ...
            group_weight = torch.tensor(config.train.group_weight).cuda().detach()
            ...
            self.groupCriterion = nn.CrossEntropyLoss(weight=group_weight)

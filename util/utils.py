...
def prep_images(images):
    ...
    """
    # 将图像除以255.0，将像素值从0-255的范围转换为0-1的范围。
    images = images.div(255.0)
    # 从图像中减去0.5，将像素值范围从0-1转换为-0.5到0.5
    images = torch.sub(images, 0.5)
    # 将图像乘以2.0，将像素值范围从-0.5到0.5转换为-1到1。
...
def show_epoch_info(phase, log_path, info):
    ...
    print_log(log_path, info['actionsAcc'], 'Group Activity Accuracy: %.2f%%, Loss: %.5f' % (
        info['activitiesAcc'], info['loss']), info['activitiesPerclassAcc'])
